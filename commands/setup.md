...
## Fallback: fuzz.sty Only

If user can't install fuzz system-wide but has the fuzz binary working:

```bash
# Copy fuzz.sty to project docs directory
cp ~/Applications/fuzz/fuzz.sty docs/

# Specs in docs/ will find it locally
```

This avoids needing `sudo make install` for the TeX path.
